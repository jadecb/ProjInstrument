...
class GuitareDAO{
    ...
    function getGuitare(int $numArticle) : array{
        ...
        $req = 'SELECT ia.numarticle, ia.nom, ia.prix, ii.materiauxprincipal, ii.couleur, ii.largeur, ii.longueur, ii.hauteur, g.materiauxManche, g.typeGuitare, g.materiauxBoitier, g.nbrCordes FROM infoArticle ia, infoInstrument ii, guitare g where ia.numArticle='.$numArticle.' AND ii.numArticle='.$numArticle.' AND g.numArticle='.$numArticle;
    ...
    function getAllGuitare() : array{
        $dao = new GuitareDAO(); // instancie l'objet DAO
        $req = 'SELECT ia.numarticle, ia.nom, ia.prix, ii.materiauxprincipal, ii.couleur, ii.largeur, ii.longueur, ii.hauteur, a.materiauxManche, a.typeGuitare, a.materiauxBoitier, a.nbrCordes FROM infoArticle ia, infoInstrument ii, Guitare a WHERE ia.numArticle=ii.numArticle AND ia.numArticle=a.numarticle';
        $sth = $this->db->query($req);
